...
const { User, Country, State, Plan, Userattendancelog, Emailtemplate, Subscriptions, Sitesettings, Usersnapshots, Roles, Break, Usersbreakslogs, Userapplist, Task, Tasksactivities, Projects, Settings, Sendmessage, Op, sequelize } = require('../../sequelize');
var p;
...
module.exports = {
  ...
  checkifemailexistforgot: async (data, callBack) => {
    await User.findAll({
      where: { email: data.email, is_detained: 'N' }
    }).then(emailexist => callBack(null, emailexist)).catch(function (err) {
      // handle error;
      return callBack(err);
    });
  ...
  savesendmessage: async (data, callBack) => {
    await Sendmessage.create({ name: data.name, email: data.email, subject: data.subject, phone: data.phone, description: data.message }).then(emailexist => callBack(null, emailexist)).catch(function (err) {
  ...
  create: async (data, callBack) => {
    ...
    await User.create({ companyname: data.companyname, firstname: data.firstname, lastname: data.lastname, email: data.email, password: data.passwords, activation_key: data.activationkey, country_id: data.country_id, state_id: data.state_id, city: data.city, mobile: data.mobile, status: statusdata, role_id: data.role_id, plan_id: data.plan_id }).then(createlist => callBack(null, createlist)).catch(function (err) {
      // handle error;
      return callBack(err);
    });
  ...
  getdemouser: async (data, callBack) => {
    ...
    await User.findAll({
      ...
      order: [[{ model: Subscriptions }, 'id', 'asc']]
  ...
  getusersearch: async (data, callBack) => {

    var cond = { role_id: 2, is_detained: 'N' }
    if (data.name) {
    ...
    if (data.country_id) {
      ...
      cond = { ...cond, country_id: data.country_id }

    }

    if (data.plan_id) {

      cond = { ...cond, plan_id: data.plan_id }

    }

    var assocond = { status: 'Y' }
    if (data.startdate) {
  ...
  getuserfortimelineattendance: async (data, callBack) => {
    const startdate = new Date();
    var TODAY_START = moment(startdate).startOf('day').format('YYYY-MM-DD HH:mm:ss')
    var TODAY_END = moment(startdate).endOf('day').format('YYYY-MM-DD HH:mm:ss')
    var date = new Date();
    ...
    await User.findAll({
      ...
      include: [
        {
          model: Usersnapshots,
          attributes: [[sequelize.fn('MAX', sequelize.col('capturetime')), 'capturetime']],
          where: {
            capturetime: {
              [Op.gt]: firstDay,
              [Op.lte]: lastDay
  ...
  viewdetailidfind: async (data, callBack) => {

    await User.findOne({
      where: { id: data.id, parent_id: data.userId },
      include: [{
        model: Roles,
        attributes: ['name']
      },
  ...
  getuserbiling: async (data, callBack) => {
    ...
    await Subscriptions.findAll({
      ...
      include: [{
        ...
        where: { role_id: 2, status: 'Y', is_detained: 'N' }
      }, {
        model: Plan,
  ...
  getuserbilingsearch: async (data, callBack) => {



    var cond = { plan_id: { [Op.notIn]: [5] } }



    if (data.status) {

      cond = { ...cond, status: data.status }
    ...
    var acond = { role_id: 2, status: 'Y', is_detained: 'N' }
    if (data.name) {
  ...
  tasklistget: async (data, callBack) => {
    ...
    await User.findOne({
      ...
      Task.findAll({
        where: {
          userId: createduser.parent_id, projects_id: {
            [Op.ne]: 0
          }
        },
  ...
  getcompanytask: async (data, callBack) => {
    ...
    await Task.findAll({
      where: {
        userId: data.userid, projects_id: {
          [Op.ne]: 0
        }
      },
  ...
  activitytasklistgetsearch: async (data, callBack) => {
  ...
  gettotaltaskcalculation: async (data, callBack) => {
    let starts = 1;

    pool.query('SELECT SUM(ABS(UNIX_TIMESTAMP(`endtime`))-ABS(UNIX_TIMESTAMP(`starttime`))) AS diff FROM `tasks_activities` where tasks_id=?',
      [starts], (error, results, fields) => {
          if (error) {
            console.log("test");
          }
 ...
 await Usersnapshots.findAll({
  ...
  monthlyproductivityget: async (data, callBack) => {
    var date = new Date();
    var firstDay = new Date(date.getFullYear(), date.getMonth(), 1);
    var lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0);
    await Usersnapshots.findAll({
      attributes: [[sequelize.fn('sum', sequelize.col('productivitytime')), 'productivitytime'], [sequelize.fn('sum', sequelize.col('totalIdleMinutes')), 'totalIdleMinutes'], 'userId','capturetime'],
      where: {
        capturetime: {
          [Op.gte]: firstDay,
          [Op.lte]: lastDay
      ...
      group: [['userId'], [sequelize.fn('DATE', sequelize.col('capturetime'))]],
      include: [{
        model: User,
        where: { parent_id: data.userid },
        attributes: ['id', 'firstname', 'lastname']
  ...
  monthlyproductivitygetsearch: async (data, callBack) => {
    var date = new Date();
...
if(data.is_screenshot){
  data.is_screenshot=data.is_screenshot;
}else{
  data.is_screenshot='Y';
}
    await User.update({ firstname: data.firstname, lastname: data.lastname, is_screenshot:data.is_screenshot,mobile: data.mobile, email: data.email }, {
      where: { id: data.userid }
    }).then(function () {
      User.findOne({
        where: { id: data.userid },
  ...
  userdeletesuperbyid: async (data, callBack) => {
    await User.update({ is_detained: 'Y', status: 'N' }, {
      where: { id: data.id }
    }).then(function () {
      User.findAll({
        where: { role_id: 2, is_detained: 'N' },
  ...
  getprojectsmainedit: async (data, callBack) => {
    await Projects.update({ name: data.name, description: data.description, status: 'Y' }, {
      where: { id: data.id }
    }).then(function () {
      Projects.findOne({
        where: { id: data.id },
  ...
  addsubscriptionsid: async (data, callBack) => {
    ...
    await User.findOne({
      ...
      User.update({ companyname: data.name, mobile: data.mobile, address: data.address, zipcode: data.zipcode }, {
        ...
        Subscriptions.create({ plan_id: data.plan_id, totaluser: data.noOfRoom, discount: data.discount, plantotalprice: data.plantotalprice, taxprice: data.taxprice, payment_detail: data.total, userId: createdUser.id, created: startdate, expiry_date: expireds, status: data.status, peruserprice: data.peruserprice, order_id: data.orderid }).then(notes => callBack(null, notes));
      }).catch(function (err) {
        // handle error;
        return callBack(err);
      })
  ...
  getplan: async (data, callBack) => {
    ...
    await User.findAll({
      ...
      where: {
        role_id: 2, status: 'Y', companyname: {
          [Op.like]: '%' + data.search + '%'
        }
      }
    }).then(getcountrylist => callBack(null, getcountrylist)).catch(function (err) {
