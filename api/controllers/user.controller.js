const { getcountry, getstate, getSearchid, getstateselect, login, create, checkifemailexist, getuser, userupdatestatusbyid, userdeletesuperbyid, getemailtemplate, getcompanysettingsid, saveuserpunch, savetaskstartid, savebreakstartid, savebreakstopid, savetaskstopid, saveuserpunchout, activationverificationid, getplan, getsubscriptionidcreated, planupgradesbyid, getDetailid, getsubscriptionid, getsubscriptiondetailid, addsubscriptionsid, getuserbiling, getadmin, editprofileid, getsitesettings, useredit, sitesettingedit, passwordedit, getuserbilingcompany, getselectedcompanydetail, updatepaymentid, datatransferid, getproductivityinfo, getproductivityinfoweb, getproductivityinfototalweb, getusercompany, addcompanyuserid, checkemailexistid, getattendence, checkiftodaydateexistuserid, getcpaturescrreninterval, getsnapshotsinfo, breaklistget, tasklistget, getapps, applisttransfer, getproductivityinfomonth, usereditprofile, activeactivitygetweb, activeactivitygetupdate, getuserfortimeline, getuserfortimelinesecond, gettimeline, companysettingsid, dailyattendanceget, getdailyattendancesearch, dailyattendancegetupdated, monthlyattendanceget, monthlyinoutget, getprojectsmain, getcompanytask, getprojectsmainadd, getcompanyprojects, gettaskview, getprojectsmainedit, projecttasklistget, activitytasklistget, getpriority, taskaddget, checkifdataexist, gettodayinfo, monthlyattendancegetnext, getapplist, gettodayproductivity, gettodayproductivityasc, getlatestsnapshot, getapplistusage, savetaskstopidmanually, gettodayproductivitytry, gettodayproductivitytrytotal, checksubscription, getuserfortotal, userupdatestatusteambyid, gettodayproductivitytrysearch, gettodayproductivitytrytotalsearch, getsnapshotsinfosearch, getlastpunchin, monthlyattendancegetsearch, monthlyinoutgetsearch, gettimelinesearch, getappssearch, getproductivityinfowebsearch, activeactivitygetwebsearch, getproductivityinfototalwebsearch, updateuserid, resetpasswordverificationid, companysettingupdatestatusid, viewdetailidfind, checkifemailexistforgot, getusersearch, getuserbilingsearch, savesendmessage, getdemouser, activitytasklistgetsearch,monthlyproductivityget,monthlyproductivitygetsearch,gettotaltaskcalculation } = require('../users/user.service');
...
const transporter = nodemailer.createTransport({
    ...
    host: "smtp.gmail.com",
    auth: {
        user: 'sales@doomshell.com',
        pass: 'ttsjlriymmsagbjs',
    },
...
module.exports = {
        ...
        gettodayproductivitytrysearch(body, (err, results) => {
            gettodayproductivitytrytotalsearch(body, (err, resultstotal) => {
                ...
                getuserfortimeline(body, (err, resultss) => {
                    ...
                    monthlyproductivitygetsearch(body, (err, resultsss) => {
                ...
                return res.status(200).json({
                    ...
                    user:uarray,
                    monthlyproductivityreport:resultsss,
        ...
        gettodayproductivitytry(body, (err, results) => {
            gettodayproductivitytrytotal(body, (err, resultstotal) => {
                ...
                getuserfortimeline(body, (err, resultss) => {
                    ...
                    monthlyproductivityget(body, (err, resultss) => {
                ...
                return res.status(200).json({
                    ...
                    datatotal: resultstotal,
                    user:uarray,
                    monthlyproductivityreport:resultss,
                    detail: ""
                });
    ...
    viewdetail: (req, res) => {
        ...
        body.userId = req.decoded.result[0].id;
        body.userid = body.id;
        viewdetailidfind(body, (err, results) => {
            if (err) {
                return res.status(500).json({
                    success: false,
                    data: [],
        ...
        viewdetailidfind(body, (err, results) => {
            ...
            getuserfortimeline(body, (err, resultss) => {
                ...
                getsnapshotsinfo(body, (err, productivityresults) => {
                    ...
                    if (productivityresults && results) {
                        return res.status(200).json({
                            success: true,
                            data: results,
                            snapshotdata: productivityresults,
    ...
    sendmessages: (req, res) => {
        ...
        savesendmessage(body, (err, results) => {
    ...
    forgotuserweb: (req, res) => {
        ...
        checkifemailexistforgot(body, (err, resultdata) => {
            if (err) {
    ...
    // Get All User
    getalldemousers: (req, res) => {


        const body = req.body;

        body.userid = req.decoded.result[0].id;
        getdemouser(body, (err, results) => {
            if (err) {
    ...
    getuserlist: (req, res) => {
        ...
        console.log(body);
        body.userid = req.decoded.result[0].id;
        getusersearch(body, (err, results) => {
            if (err) {
            ...
            getcompanyprojects(body, (err, results) => {
                projecttasklistget(body, (err, resultsproject) => {
                    getpriority(body, (err, resultspriority) => {
                        getuserfortimeline(body, (err, getuser) => {
                     gettotaltaskcalculation(body, (err, getusetaskr) => {
                            ...
                            return res.status(200).json({
                                ...
                                getuse: getuser,
                                gettotaltaskcalculation:getusetaskr
                            });
    ...
    gettaskviewsearch: (req, res) => {


        const body = req.body;
        body.startdate = moment(body.startdate).startOf('day').format('YYYY-MM-DD HH:mm:ss');
        body.enddate = moment(body.enddate).endOf('day').format('YYYY-MM-DD HH:mm:ss');
        startdates = body.startdate;
        enddates = body.enddate;
        body.userId = req.decoded.result[0].id;
        body.userid = req.decoded.result[0].id;
        ids = body.id;
        getuserfortimelinesecond(body, (err, uarray) => {
            ...
            gettaskview(body, (err, results) => {
                activitytasklistgetsearch(body, (err, resultsproject) => {
    ...
    gettimelineport: (req, res) => {
        ...
        getuserfortimeline(body, (err, resultss) => {
            ...
            body.user = uarray;
            gettimelinesearch(body, (err, results) => {
    ...
    getbillingdetail: (req, res) => {
        ...
        body.userid = req.decoded.result[0].id;
        getuserbilingsearch(body, (err, results) => {
            if (err) {
        ...
        monthlyattendancegetnext(body, (err, results) => {
            ...
            for (let hd = 0; hd < test.length; hd++) {
                ...
                if (results[hd].punch_out) {
                    ...
                    objs.push({ "date": moment(results[hd].punch_in).format('DD-MM-YYYY'), "punchIn": moment(results[hd].punch_in).format('DD-MM-YYYY HH:mm:ss'), "punchOut": moment(results[hd].punch_out).format('HH:mm:ss'), "duration": duration });
                } else {
